...
import type { SolaceConfig } from '../../shared/schema';
...
const testSolaceCredentials: SolaceConfig = {
...
async function runSolaceConnectionTests() {
  ...
  try {
    ...
    try {
      ...
      const error = err as Error;
      console.error(`Failed to subscribe to signal/output: ${error.message} ❌`);
      throw error;
    }
    ...
    try {
      ...
      const error = err as Error;
      console.error(`Failed to publish stock market data: ${error.message} ❌`);
    }
    ...
    try {
      ...
      const error = err as Error;
      console.error(`Failed to publish index market data: ${error.message} ❌`);
    }
    ...
    try {
      ...
      const error = err as Error;
      console.error(`Failed to publish signal data: ${error.message} ❌`);
    }
    ...
    try {
      ...
      const error = err as Error;
      console.error(`Failed to unsubscribe from signal/output: ${error.message} ❌`);
    }
    ...
    const error = err as Error;
    console.error(`\nSOLACE CONNECTION TEST FAILED: ${error.message}\n`);
...
export default runSolaceConnectionTests;